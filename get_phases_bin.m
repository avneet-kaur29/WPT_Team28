function [phases_BIN] = get_phases_bin(f_c_PH, grid_dim_PH, T_x_PH, foc_pt_PH, plt_y_n)
...
% coordinates), this function decides whether the phase of each element
% in the RIS should be 0 deg or 180 deg in order to focus the reflection
% on the desired location. It returns a matrix of phase values,each in
% the location of the element it corresponds to on the RIS.
%-----------------------------------------------------------------------
% INPUTS: 
%--------
...
% RIS grid, and are restricted to either 0 deg or 180 deg.
...
% To hold binary phase values
phases_BIN = zeros(size(phases_PH));

for i = 1:m_PH
    for j = 1:n_PH

        if (phases_PH(i,j) >= 0.0 && phases_PH(i,j) < 90.0) || (phases_PH(i,j) > 270.0...
            && phases_PH(i,j) < 360.0)

            phases_BIN(i,j) = 0.0;

        elseif (phases_PH(i,j) >= 90.0 && phases_PH(i,j) <= 270.0 ) 

            phases_BIN(i,j) = 180.0;
        
        else

            error("Invalid continuous phase value. Continuous phase must"...
                +" be in [0.0, +360.0); phases_PH(%d, %d) = %d.", i, j, phases_PH(i,j));
